...
class Underwriter:
    ...
    def match(self, aggregated: FrameLike) -> MatchResult:
    ...
    def recombine(self, applied: FrameLike) -> Decision:
    ...
    def underwrite(self, aggregated: FrameLike) -> Decision:
        """The whole engine: ``match`` then ``decide``."""
        return self.decide(self.match(aggregated))

    def run(self, claims: FrameLike) -> Decision:
...
def _validated_loading(loading: pl.DataFrame) -> pl.DataFrame:
    ...
    if bands.height == 0 or bands["at_least"].null_count():
        raise RulebookError("loading_table needs at least one band with a non-null `at_least`.")
    if bands["at_least"].n_unique() != bands.height:
        raise RulebookError("loading_table has duplicate `at_least` bounds.")
    if int(bands["at_least"][0]) != 0:
        raise RulebookError("loading_table's first `at_least` must be 0.")
    return bands
